...
static gboolean
panel_frame_source_prepare (GSource *source,
                            gint    *timeout_)
{
   PanelFrameSource *fsource = (PanelFrameSource *)(gpointer)source;
   gint64 current_time;
   guint elapsed_time;
   guint new_frame_num;
   guint frame_time;
...
static gboolean
panel_frame_source_check (GSource *source)
{
   gint timeout_;
   return panel_frame_source_prepare(source, &timeout_);
}

static gboolean
panel_frame_source_dispatch (GSource     *source,
...
static GSourceFuncs source_funcs = {
   panel_frame_source_prepare,
   panel_frame_source_check,
   panel_frame_source_dispatch,
};

/**
 * panel_frame_source_add:
 ...
 */
guint
panel_frame_source_add (guint       frames_per_sec,
                        GSourceFunc callback,
                        gpointer    user_data)
{
   PanelFrameSource *fsource;
   GSource *source;
   guint ret;

   g_return_val_if_fail (frames_per_sec > 0, 0);

   source = g_source_new (&source_funcs, sizeof (PanelFrameSource));
   fsource = (PanelFrameSource *)(gpointer)source;
   fsource->fps = frames_per_sec;
   fsource->frame_count = 0;
   fsource->start_time = g_get_monotonic_time () / 1000L;
   g_source_set_callback (source, callback, user_data, NULL);
   g_source_set_name (source, "PanelFrameSource");
...
guint
panel_frame_source_add_full (gint           priority,
