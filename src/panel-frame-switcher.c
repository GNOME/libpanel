...
ensure_indicator (PanelFrameSwitcher *self)
{
  ...
  if (orientation == GTK_ORIENTATION_HORIZONTAL)
    {
      cairo_move_to (cr, INDICATOR_SIZE/2-1, 0);
      cairo_line_to (cr, INDICATOR_SIZE/2-1, INDICATOR_SIZE/2);
      cairo_stroke (cr);
    ...
    {
      cairo_move_to (cr, 0, INDICATOR_SIZE/2-1);
      cairo_line_to (cr, INDICATOR_SIZE/2, INDICATOR_SIZE/2-1);
      cairo_stroke (cr);
      cairo_arc (cr, INDICATOR_SIZE-5, INDICATOR_SIZE/2-1, 4, 0, 2 * G_PI);
      cairo_fill (cr);
    }
      ...
      if (orientation == GTK_ORIENTATION_HORIZONTAL)
        {
          ...
          y = alloc.size.height - INDICATOR_SIZE - 4;
