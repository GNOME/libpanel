...
#include "panel-signal-group-private.h"

/**
 * SECTION:draft-signal-group
 * @title: PanelSignalGroup
 * @short_description: Manage a collection of signals on a #GObject
 *
 * #PanelSignalGroup manages to simplify the process of connecting
...
struct _PanelSignalGroupClass
{
  GObjectClass parent_class;

  void (*bind) (PanelSignalGroup *self,
                GObject        *target);
};

typedef struct
{
  PanelSignalGroup *group;
  gulong          handler_id;
  GClosure       *closure;
  guint           signal_id;
...
G_DEFINE_TYPE (PanelSignalGroup, panel_signal_group, G_TYPE_OBJECT)
...
static void
panel_signal_group_set_target_type (PanelSignalGroup *self,
                                    GType             target_type)
{
...
static void
panel_signal_group_gc_handlers (PanelSignalGroup *self)
{
  g_assert (PANEL_IS_SIGNAL_GROUP (self));
...
static void
panel_signal_group__target_weak_notify (gpointer  data,
                                        GObject  *where_object_was)
{
  PanelSignalGroup *self = data;
...
static void
panel_signal_group_bind_handler (PanelSignalGroup *self,
                         ...
                         GObject          *target)
{
  ...
  g_object_weak_ref (hold, panel_signal_group__target_weak_notify, self);
  ...
  for (guint i = 0; i < self->handlers->len; i++)
    {
      ...
      panel_signal_group_bind_handler (self, handler, hold);
    }
  ...
  if (target != NULL)
    {
      ...
      g_object_weak_unref (target,
                           panel_signal_group__target_weak_notify,
                           self);
    }
...
static gboolean
panel_signal_group_check_target_type (PanelSignalGroup *self,
                                      gpointer          target)
{
  if ((target != NULL) &&
      !g_type_is_a (G_OBJECT_TYPE (target), self->target_type))
    {
      g_critical ("Failed to set PanelSignalGroup of target type %s "
...
/**
 * panel_signal_group_block:
 * @self: the #PanelSignalGroup
 *
...
/**
 * panel_signal_group_set_target:
 * @self: the #PanelSignalGroup.
 * @target: (nullable) (type GObject): The target instance used
 *     when connecting signals.
 *
 * Sets the target instance used when connecting signals. Any signal
 * that has been registered with panel_signal_group_connect_object() or
                               ...
                               gpointer          target)
{
  ...
  if (self->has_bound_at_least_once)
    panel_signal_group_unbind (self);
...
static void
panel_signal_group_dispose (GObject *object)
{
  PanelSignalGroup *self = (PanelSignalGroup *)object;
  ...
  G_OBJECT_CLASS (panel_signal_group_parent_class)->dispose (object);
}

static void
panel_signal_group_finalize (GObject *object)
{
  PanelSignalGroup *self = (PanelSignalGroup *)object;

  g_weak_ref_clear (&self->target_ref);

  G_OBJECT_CLASS (panel_signal_group_parent_class)->finalize (object);
}

static void
panel_signal_group_get_property (GObject    *object,
                                 guint       prop_id,
                                 GValue     *value,
                                 GParamSpec *pspec)
{
  PanelSignalGroup *self = PANEL_SIGNAL_GROUP (object);
...
static void
panel_signal_group_set_property (GObject      *object,
                                 guint         prop_id,
                                 const GValue *value,
                                 GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_TARGET:
      panel_signal_group_set_target (self, g_value_get_object (value));
      break;

    case PROP_TARGET_TYPE:
      panel_signal_group_set_target_type (self, g_value_get_gtype (value));
      break;
...
static void
panel_signal_group_class_init (PanelSignalGroupClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->constructed = panel_signal_group_constructed;
  object_class->dispose = panel_signal_group_dispose;
  object_class->finalize = panel_signal_group_finalize;
  object_class->get_property = panel_signal_group_get_property;
  object_class->set_property = panel_signal_group_set_property;

  /**
   * PanelSignalGroup:target
   *
  ...
  /**
   * PanelSignalGroup::unbind:
   * @self: a #PanelSignalGroup
   *
   * This signal is emitted when the target instance of @self
   * is set to a new #GObject.
   *
...
static void
panel_signal_group_init (PanelSignalGroup *self)
{
...
/**
 * panel_signal_group_new:
 * @target_type: the #GType of the target instance.
 *
 * Creates a new #PanelSignalGroup for target instances of @target_type.
 *
 * Returns: a new #PanelSignalGroup
 */
PanelSignalGroup *
panel_signal_group_new (GType target_type)
{
  g_return_val_if_fail (g_type_is_a (target_type, G_TYPE_OBJECT), NULL);

  return g_object_new (PANEL_TYPE_SIGNAL_GROUP,
                       "target-type", target_type,
                       NULL);
}

static void
panel_signal_group_connect_full (PanelSignalGroup *self,
                                 ...
                                 gboolean          is_object)
{
  ...
  target = g_weak_ref_get (&self->target_ref);
  if (target != NULL)
    panel_signal_group_bind_handler (self, handler, target);
...
/**
 * panel_signal_group_connect_object: (skip)
 * @self: a #PanelSignalGroup
 ...
 */
void
panel_signal_group_connect_object (PanelSignalGroup *self,
                                   const gchar      *detailed_signal,
                                   GCallback         c_handler,
                                   gpointer          object,
                                   GConnectFlags     flags)
{
  ...
  panel_signal_group_connect_full (self, detailed_signal, c_handler, object, NULL,
                                 flags, TRUE);
}

/**
 * panel_signal_group_connect_data:
 * @self: a #PanelSignalGroup
 * @detailed_signal: a string of the form "signal-name::detail"
 ...
 */
void
panel_signal_group_connect_data (PanelSignalGroup *self,
                                 const gchar      *detailed_signal,
                                 GCallback         c_handler,
                                 gpointer          data,
                                 GClosureNotify    notify,
                                 GConnectFlags     flags)
{
  panel_signal_group_connect_full (self, detailed_signal, c_handler, data, notify,
                                 flags, FALSE);
}

/**
 * panel_signal_group_connect: (skip)
 * @self: a #PanelSignalGroup
 * @detailed_signal: a string of the form "signal-name::detail"
 * @c_handler: (scope notified): the #GCallback to connect
 * @data: the data to pass to @callback calls
 ...
 */
void
panel_signal_group_connect (PanelSignalGroup *self,
                            const gchar      *detailed_signal,
                            GCallback         c_handler,
                            gpointer          data)
{
  panel_signal_group_connect_full (self, detailed_signal, c_handler, data, NULL,
                                 0, FALSE);
}

/**
 * panel_signal_group_connect_after: (skip)
 * @self: a #PanelSignalGroup
 * @detailed_signal: a string of the form "signal-name::detail"
 * @c_handler: (scope notified): the #GCallback to connect
 * @data: the data to pass to @callback calls
 ...
 */
void
panel_signal_group_connect_after (PanelSignalGroup *self,
                                  const gchar      *detailed_signal,
                                  GCallback         c_handler,
                                  gpointer          data)
{
  panel_signal_group_connect_full (self, detailed_signal, c_handler,
                                 data, NULL, G_CONNECT_AFTER, FALSE);
}

/**
 * panel_signal_group_connect_swapped:
 * @self: a #PanelSignalGroup
 * @detailed_signal: a string of the form "signal-name::detail"
 ...
 */
void
panel_signal_group_connect_swapped (PanelSignalGroup *self,
                                    const gchar      *detailed_signal,
                                    GCallback         c_handler,
                                    gpointer          data)
{
